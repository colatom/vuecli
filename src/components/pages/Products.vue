<template>
    <div>
        ...
        <div class="text-right mt-3">
            <button class="btn btn-primary" @click="openModal(true)">建立新的商品</button>
        </div>
        <table class="table mt-4">
            ...
            <tbody>
                <tr v-for="item in products" :key="item.id">
                    <td>{{ item.category }}</td>
                    <td>{{ item.title }}</td>
                    <td class="text-right">{{ item.origin_price | currency }} / {{ item.unit }}</td>
                    <td class="text-right">{{ item.price |currency }} / {{ item.unit }}</td>                    
                    <td>
                        <span v-if="item.is_enabled" class="text-success">啟用</span>
                        <span v-else class="text-unsuccess">未啟用</span>
                    </td>
                    <td>
                        <button class="btn btn-outline-primary btn-sm" @click="openModal(false, item)">編輯</button>
                        <button class="btn btn-outline-danger btn-sm" @click="delProductModal(true, item)">刪除</button>
                    </td>
                </tr>
            </tbody>
        </table>
  ...
  aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            ...
            <div class="modal-body">
                ...
                <div class="col-sm-4">
                    ...
                    <input type="text" class="form-control" id="image" v-model="temProduct.imageUrl"
                        placeholder="請輸入圖片連結">
                    </div>
                    <div class="form-group">
                    ...
                    class="img-fluid" :src="temProduct.imageUrl" alt="">
                </div>
                <div class="col-sm-8">
                    <div class="form-group">
        ...
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                是否刪除 <strong class="text-danger">{{ temProduct.title }}</strong> 商品(刪除後將無法恢復)。
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">取消</button>
